...
recupChar(L,I,T):-
    (   I=< 159 ->      %On fait tourner 160 fois la fonction car il y a 10 possibilités de victoire (4 lignes, 4 colonnes, 2 diagonales)
...
tourJeu(L,T,WinState,J,Difficulte):- 
    ...
    listeLettresNJ(NewListe),       %L = liste des pièces sur la grille de jeu, T = tour de jeu
                                    %WinState = état de la partie (0=personne n'a gagné, 1=il y a un gagnant), J = liste des pièces Jouables
    ...
    compteOccurence(Sortie,b,0,0,OccurenceB),   %On compte l'occurence de chaque lettre
    ...
    plusPresente(OccurenceB,OccurenceN,OccurenceC,OccurenceR,OccurenceH,OccurenceS,OccurenceP,OccurenceT);  %On affiche un message en fonction des lettres les plus présentes
...
evaluationLettre(E,I,ListeL,NewListeF):-
    ...
    append(ListeL,[Char1],NewListe1),       %On les ajoute à la liste des lettres
...
plusPresente(B,N,C,R,H,S,P,T):-
    ...
    (   max_member(B, L) ->  write('Piece Blanche'),nl;write('')),  %On affiche uniquement les caractéristiques les plus présentes
...
jouer:- 
    ...
    write('Veuillez selectionner une difficulte pour le jeu :'),
    nl,
    nl,
